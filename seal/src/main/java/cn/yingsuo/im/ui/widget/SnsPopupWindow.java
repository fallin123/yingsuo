...
public class SnsPopupWindow extends PopupWindow implements OnClickListener{
	...
	public SnsPopupWindow(Context context) {
		View view = LayoutInflater.from(context).inflate(R.layout.popu_frend_quan_more, null);
		digBtn = (TextView) view.findViewById(R.id.popu_quan_zan_tv);
		commentBtn = (TextView) view.findViewById(R.id.popu_quan_pinglun_tv);
		digBtn.setOnClickListener(this);
		commentBtn.setOnClickListener(this);

		this.setContentView(view);
		this.setWidth(DisplayUtils.dip2px(context, 100));
		this.setHeight(DisplayUtils.dip2px(context, 30));
	...
	public void onClick(View view) {
		dismiss();
		switch (view.getId()) {
		case R.id.popu_quan_zan_tv:
			mItemClickListener.onItemClick(mActionItems.get(0), 0);
			break;
		case R.id.popu_quan_pinglun_tv:
