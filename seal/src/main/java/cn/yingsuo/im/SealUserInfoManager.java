...
import cn.yingsuo.im.db.BlackList;
import cn.yingsuo.im.db.BlackListDao;
import cn.yingsuo.im.db.DBManager;
import cn.yingsuo.im.db.Friend;
import cn.yingsuo.im.db.FriendDao;
import cn.yingsuo.im.db.GroupMember;
import cn.yingsuo.im.db.GroupMemberDao;
import cn.yingsuo.im.db.Groups;
import cn.yingsuo.im.db.GroupsDao;
import cn.yingsuo.im.db.UserInfoBean;
...
public class SealUserInfoManager implements OnDataListener {
    ...
    private boolean fetchFriends() throws HttpException {
        ...
        if (userRelationshipResponse != null && userRelationshipResponse.getCode() == 200) {
            List<NewFrendResultResponse> list = userRelationshipResponse.getRes();
     ...
     */
    private List<Friend> addFriends(final List<NewFrendResultResponse> list) {
        if (list != null && list.size() > 0) {
            List<Friend> friendsList = new ArrayList<>();
            for (NewFrendResultResponse resultEntity : list) {
               /* if (resultEntity.getStatus() == 20) {*/
                Friend friend = new Friend(
                        resultEntity.getId(),
                        resultEntity.getNi_name(),
                        Uri.parse(resultEntity.getHead_img()),
                        resultEntity.getNi_name(),
                        null, null, null, null,
                        CharacterParser.getInstance().getSpelling(resultEntity.getNi_name()),
                        CharacterParser.getInstance().getSpelling(resultEntity.getNi_name()));
                if (friend.getPortraitUri() == null || TextUtils.isEmpty(friend.getPortraitUri().toString())) {
