package cn.yingsuo.im.server.utils.photo;
...
public class PhotoUtils {
     ...
     * 存放拍照的图片
     */
    private String cameraPicPath = "crop_file.jpg";
     ...
     */
    @SuppressLint("InlinedApi")
    public void selectAllPicture(Activity activity, int pciNum) {
        ImageSelectorUtils.openPhoto(activity, INTENT_SELECT, false, pciNum);
    }
    ...
    private Uri buildUri(Activity activity) {
        if (CommonUtils.checkSDCard()) {
            return Uri.fromFile(Environment.getExternalStorageDirectory()).buildUpon().appendPath(cameraPicPath).build();
        } else {
            return Uri.fromFile(activity.getCacheDir()).buildUpon().appendPath(cameraPicPath).build();
        }
    }
     ...
     * @param requestCode
     */
    public void onActivityResultForCamera(Activity activity, int requestCode) {
        if (onPhotoResultListener == null) {
            Log.e(tag, "onPhotoResultListener is not null");
            return;
        }
        ...
        switch (requestCode) {
            //拍照
            case INTENT_TAKE:
                String path = buildUri(activity).getPath();
